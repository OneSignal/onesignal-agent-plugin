...
- **Open the page yourself when the session exposes a browser tool** (SKILL.md "Inputs you need"): load the served URL in it, then read `OneSignal.User.PushSubscription.id` and `.optedIn` from the page for step 2 — no REST key needed. When the setup skill wrote the integration, its debug helper opens the OneSignal slidedown about 2 seconds after init; the user clicks Allow on the slidedown and then in the browser's native dialog. Tell them that once, then read the values. The native dialog is outside the page, so no tool clicks it.
- Without a browser tool, instruct the user to open a fresh tab (not just refresh; a new tab triggers full SDK init), click Allow on the slidedown and in the native dialog, then poll step 2.
