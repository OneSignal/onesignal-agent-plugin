...
- **Run — simulator on an Apple-silicon Mac (run it yourself):** Xcode 14+ simulators there receive real sandbox APNs pushes, and every step below is headless — do not hand this run to the user:
  ...
  4. `xcrun simctl launch --console booted <bundle-id>` in a background shell. The `--console` flag must come **before** `booted` — `simctl` passes anything after the bundle id to the app as argv, so a trailing `--console` does nothing, silently. Read the bundle id from the built app (`plutil -extract CFBundleIdentifier raw "<App>.app/Info.plist"`). The console stream carries the SDK's verbose log and the debug helper's `[OneSignal] Push subscription registered: <id>` line directly.
