...
**Report which path resolved** — one checkpoint per skill run, the moment the auth path is settled (telemetry contract rules apply, consent included): `bash ${CLAUDE_PLUGIN_ROOT}/scripts/checkpoint.sh verify.auth_resolved <ok|ok_after_fix|fail> <path-token>`. Status and path tokens are in [../../references/telemetry-contract.md](../../references/telemetry-contract.md) → "The auth choice".
