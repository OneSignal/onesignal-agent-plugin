...
- **Run the app yourself when a simulator or a local server can serve.** On an Apple-silicon Mac, boot, install, and launch the iOS simulator headlessly (exact `simctl` sequence in platform-verification.md Part A) — do not ask the user to do a run you can do. A physical phone is different: you cannot drive it, so hand that launch to the user and wait.
- **You cannot press "Allow" anywhere — simulator included** (`xcrun simctl privacy` has no notifications service). On iOS the prompt does not gate step 2: with the setup skill's `UIBackgroundModes = remote-notification` edit, the SDK creates the server-side subscription before the prompt is answered (SDK-verified; detail in platform-verification.md Part A). The accepted prompt is needed from step 4 on — tell the user the single tap you need from them, and when.
...
- **Poll loop:** every ~5s, up to a **2-minute timeout**. Between polls: if you launched the app yourself (iOS simulator), watch the `--console` output for the debug helper's registration line instead of nagging; otherwise remind the user to launch the app on a real device/browser and accept the notification permission prompt.
- **iOS: presence here does not prove the prompt was accepted.** The subscription registers before the tap (see step 1), so a non-empty poll on iOS means "device registered", not "user opted in" — the tap is still required before the step-4 send.
...
Send to ONLY the subscription from step 2 — never a broadcast. **Ask before sending:** this is a real, visible push to a real device and an action on the user's live OneSignal app — state the target subscription id and the App ID, and get an explicit yes first. Never send without it.
