...
3. **OneSignal MCP tools — only on app match.** If OneSignal MCP tools are connected, check first (`onesignal_config` / `list_apps`) that the connection can access the App ID you are checking, and use the tools ONLY on a match. The connection is an OAuth grant that follows the signed-in account's permissions and may cover many apps; a connection whose account cannot see this app is not a credential for it — note the mismatch in one line and keep resolving; never report a *different* app's data as the user's status. If the tools are absent, check for a registered-but-unauthenticated server (the plugin bundles it in `.mcp.json`; in Claude Code, `claude mcp list` shows it as needing authentication). Offer it as the recommended path: ask the user to run `/mcp` → **onesignal** → **Authenticate**. Say what the flow does: the browser opens OneSignal's first-party sign-in page; the user signs in and approves access. No App ID or REST API key is entered, so no key touches the chat transcript, and the grant is revocable from **Connected apps** in the account settings (README "Optional: connect the OneSignal MCP server"). After the user authenticates, the app-match rule above applies unchanged.
