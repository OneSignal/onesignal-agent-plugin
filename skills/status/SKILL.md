...
3. **OneSignal MCP tools — only on app match.** If OneSignal MCP tools are connected, call `onesignal_config` first and use them ONLY if they are bound to the same App ID you are checking. A connected server pointed at a different app or account is not a credential for this app — note the mismatch in one line and keep resolving; never report *its* app list or data as the user's status. If the tools are absent, check for a registered-but-unauthenticated server (the plugin bundles it in `.mcp.json`; in Claude Code, `claude mcp list` shows it as needing authentication). Offer it as the recommended path: ask the user to run `/mcp` → **onesignal** → **Authenticate**. Say the tradeoff plainly: the hosted **Connect OneSignal** page collects the App ID and a REST API key, and Smithery stores both — the key stays out of the chat transcript (README "Optional: connect the OneSignal MCP server"). After the user authenticates, the app-match rule above applies unchanged.
