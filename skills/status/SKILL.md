...
3. **OneSignal MCP tools — only on app match.** If OneSignal MCP tools are connected, check first with `list_apps` (paginated — page until the items seen equal the response's `total_count` before you conclude absence) that the OAuth grant can access the App ID you are checking, and use the tools ONLY on a match, with that `app_id` passed on every call. (`onesignal_config` reports connection details, not app membership — it is not this check.) The connection is an OAuth grant that follows the signed-in account's permissions and may cover many apps; a connection whose account cannot see this app is not a credential for it — note the mismatch in one line and keep resolving; never report a *different* app's data as the user's status. If the tools are absent, check for a registered-but-unauthenticated server (the plugin bundles it in `.mcp.json`; in Claude Code, `claude mcp list` shows it as needing authentication). Offer it as the recommended path: ask the user to run `/mcp` → **onesignal** → **Authenticate**. Say what the flow does: the browser opens OneSignal's first-party sign-in page; the user signs in and approves access. No App ID or REST API key is entered, so no key touches the chat transcript, and the grant is revocable from **Connected apps** in the account settings (README "Optional: connect the OneSignal MCP server"). After the user authenticates, the app-match rule above applies unchanged.
4. **Ask.** If the MCP server is absent from the session, or the user declines it, ask the user for the App ID and an app-scoped key (the same pair setup uses) and continue. Do not proceed guessing.

If you have an App ID but **no key**, offer the MCP connection first (Step 0 item 3) — an authenticated MCP session can probe rungs 4–6 immediately, plus the web half of rung 1 through the unauthenticated sync probe. Even with the MCP, rungs 2–3 stay ⚠️ unknown, and so does the app-config half of rung 1: the view-app read, the subscription poll, and the rung-3 sample all need a key. With no key and no MCP, only the **web half of rung 1** is measurable (the unauthenticated sync probe in signals-reference.md). Render every unmeasured rung as ⚠️ unknown, say which auth path would unlock which rungs, and ask for the key (Step 0 item 4) — do not fabricate pass/fail.
...
|---|---|---|---|
| 1 | **SDK initialized / platforms configured** | App config shows ≥1 platform configured (has FCM/APNs/web keys) | app config via the view-app read (key) or the web sync probe (no key) |
