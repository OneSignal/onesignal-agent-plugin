...
- `OneSignal.Notifications.requestPermission(true)` called from a coroutine — the ONLY permission prompt.
...
- Log the subscription ID exactly once (an `AtomicBoolean` logged-once guard), then remove the observer.
