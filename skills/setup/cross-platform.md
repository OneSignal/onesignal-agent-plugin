...
Reference for the `setup` skill. Follow [SKILL.md](SKILL.md) Steps 0–8; this file covers the wrapper frameworks. `<plugin>` in the commands below is the directory SKILL.md defines: walk up from this file's directory to the first directory that contains a `scripts/` folder. All of these wrap the native iOS + Android SDKs, so **the iOS-native human column applies in full** (Apple portal `.p8` + Xcode capabilities/NSE for rich features) and Android needs the FCM v1 service-account JSON — see [../../references/platform-matrix.md](../../references/platform-matrix.md), and cross-reference [ios.md](ios.md) / [android.md](android.md) for the native details. Do not contradict the matrix.
