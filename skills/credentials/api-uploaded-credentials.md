...
Both credentials are Base64-encoded before upload and validated server-side on upload. Upload goes to the **write-once provisioning endpoint** `POST /api/v1/apps/{app_id}/credentials`, authenticated with an **app-scoped key** (`Authorization: Key <key>` — the onboarding setup token or the app's REST API key). An org key is only needed to *replace* existing credentials, which happens via the dashboard or the org-key update API, never through this endpoint. See [../../references/api-reference.md](../../references/api-reference.md) for the full contract and [../../references/safety-contract.md](../../references/safety-contract.md) for secret handling.
