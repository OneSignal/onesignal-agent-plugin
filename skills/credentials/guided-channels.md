...
If the web platform has never been configured, the agent can set it via the write-once provisioning endpoint (`POST /api/v1/apps/{app_id}/credentials`). Web is a third call site for the same tool, so use the **same transport rules as the other credentials** — the MCP `provision_app_credentials` tool vs. a direct app-scoped-key `POST`, and the **App-ID precondition** for the MCP tool — owned by [SKILL.md](SKILL.md) → "The API-upload mechanism" (don't restate them here). The web-specific params: `chrome_web_origin` (must be `https://` and exactly match the origin) plus optional `chrome_web_default_notification_icon`. That endpoint does NOT take Site Name or any Safari params — those, and any later changes to an already-configured web platform, happen in the dashboard (Settings → Push & In-App → Web).
