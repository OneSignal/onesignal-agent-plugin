...
The probe recipes are owned by the status skill — [../status/signals-reference.md](../status/signals-reference.md#rung-1--sdk-initialized--platforms-configured) (rung 1) defines the reads and the response semantics, and `${CLAUDE_PLUGIN_ROOT}/scripts/onesignal_api.py` encodes them as commands. Use those; do not hand-roll the calls.

1. **Push platforms (iOS / Android):** run `onesignal_api.py app <app_id>` — the view-app read, `GET /api/v1/apps/{app_id}` — with an app-scoped key (the script takes `--key` or reads `$ONESIGNAL_REST_API_KEY` / `$ONESIGNAL_SETUP_TOKEN`). No MCP tool returns the per-app platform config (api-reference.md), so this read has no MCP path. Populated credential fields for the target platform mean the platform is configured (rung-1 recipe). The script reports only the response's field names; if those alone cannot decide, issue the raw `GET` and inspect the target platform's fields.
2. **Web:** run `onesignal_api.py web-probe <app_id>` — no key needed. The script wraps the unauthenticated sync probe and always appends the throwaway `?fresh=` param that bypasses the ~1 h CDN cache (api-reference.md). `success: true` means the web platform is provisioned.
...
6. **Check cannot run** (no key available, or the `GET` itself fails) **→** say so and continue into the flow anyway. The write-once endpoint still guards the case: an upload against an already-configured platform returns a 409, and the validation loop maps it.
