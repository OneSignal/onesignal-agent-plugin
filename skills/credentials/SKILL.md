...
1. **Push platforms (iOS / Android):** call `GET /api/v1/apps/{app_id}` with an app-scoped key (`Authorization: Key <key>` — the setup key from the invocation, or `$ONESIGNAL_REST_API_KEY`). No MCP tool returns the per-app platform config (see api-reference.md), so this read is always a direct `GET`. Populated credential fields for the target platform in the response mean the platform is configured.
2. **Web:** use the unauthenticated probe `GET https://api.onesignal.com/sync/{app_id}/web?fresh=<timestamp>` — `success: true` means the web platform is provisioned. Keep the throwaway query param; api-reference.md documents the ~1 h CDN cache this bypasses.
